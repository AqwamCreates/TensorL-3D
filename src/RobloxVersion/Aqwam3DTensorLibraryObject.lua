...
local function sum(tensor, dimension)
	...
	local dimensionArray = getSize(tensor)
...
function AqwamTensorLibrary3D.createTensorFromFunction(dimensionArray, functionToUse)
...
function AqwamTensorLibrary3D:expand(dimensionArray)
	
	throwErrorIfDimensionArrayLengthIsNotEqualToThree(dimensionArray)
	
	local newTensor = expandTensor(self, dimensionArray)
	...
	for i = 1, dimensionArray1[1], 1 do
		...
		for j = 1, dimensionArray1[2], 1 do
			...
			for k = 1, dimensionArray2[3], 1 do
				...
				for l = 1, dimensionArray1[3] do sum = sum + (self[i][j][l] * other[i][l][k]) end
